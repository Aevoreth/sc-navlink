...
public sealed class TourController
{
    ...
    [
        new(TutorialTarget.None, "Already working",
            "SC-navLink went live the moment you launched it - your game feeds it everything through one log file, no setup required. This quick tour shows you the map: a status strip that reports, a module dock that works, and an overlay that flies with you."),
        new(TutorialTarget.SessionPill, "The status strip",
            "This pill lights up when Star Citizen writes to Game.log and SC-navLink reads along - read-only, nothing modified, EAC-safe. If it stays dark, point Settings at your Game.log path."),
        ...
        new(TutorialTarget.RefineryTile, "Refinery",
            "Click + Add work order inside and SC-navLink counts down every job. The pill on this tile lights up when orders are ready to collect."),
        ...
        new(TutorialTarget.OpenOverlay, "The third space",
            "Click here to launch the overlay - a compact panel that floats over Star Citizen so you never leave the game to use SC-navLink."),
        new(TutorialTarget.OverlayHub, "Proof of life",
            "The overlay opens on the HUB: the green light means SC-navLink is reading a live game session, and the location beside it glows cyan while the game knows where you are - grey means it is your last known spot. Refinery, auto-load and hangar cards, your money, server and shard read out below."),
        new(TutorialTarget.ScanToggle, "Auto-scan is opt-in",
            "Switch this on and SC-navLink reads rock signatures straight off your screen through the magenta box you draw once. It pauses on its own whenever you and the game are both in the background."),
