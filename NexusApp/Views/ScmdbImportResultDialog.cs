...
/// ToImport bucket only after this returns true. Sections: a "WILL BE MARKED OWNED" scrollable
/// names list (the ToImport bucket - what confirming will actually do, mirroring
/// ImportResultDialog's own matched-names list verbatim, header wording included), summary lines
/// for already-owned (count only - the Game.log sibling has no such bucket to mirror the list-vs-
/// count choice from, so this stays a count, unchanged from before this list was added),
...
{
    public ScmdbImportResultDialog(IReadOnlyList<string> toImport, int alreadyOwned, IReadOnlyList<string> unrecognized,
        int skippedNotCompleted, int malformedEntries, int missionCount, bool newerVersion)
    {
        ...
        int toImportCount = toImport.Count;
    ...
    // Read-only, selectable, scrollable box of a full name list (ToImport or unrecognized) - same
    // MaxHeight/scroll-cap idiom as ImportResultDialog's own NamesBox, for long lists alike.
