**SC-navLink __VERSION__** - an unofficial, fan-made operations companion for Star Citizen.

Both downloads are self-contained, because SC-navLink includes the .NET runtime. Both downloads:
...
### Option 1 - Installer (`SC-navLink_Setup.exe`) - *recommended, user friendly*
A guided setup installs SC-navLink like normal Windows software.

1. Download **`SC-navLink_Setup.exe`** below.
...
4. Open SC-navLink from the Start menu or the desktop.

- The installer makes a Start-menu shortcut and an optional desktop shortcut.
- You can uninstall SC-navLink cleanly from *Add or remove programs*.
- SC-navLink installs for the current user under `%LOCALAPPDATA%`. This needs no admin rights.

### Option 2 - Portable (`sc-navlink_portable.zip`)
Run SC-navLink directly. SC-navLink needs no installation.

1. Download **`sc-navlink_portable.zip`** below.
...
4. Open the extracted folder. Then open the **sc-navlink** folder inside it. Double-click **`SC-navLink.exe`**.

- SC-navLink needs no installation. It writes nothing to the registry.
- SC-navLink stores settings and work orders under `%APPDATA%\sc-navlink`. To remove SC-navLink completely, delete the app folder and that data folder.
- With update checks on, the portable can install a new version in place and restart. Updates stay off until you say yes.
- You can move SC-navLink between PCs. You can also run SC-navLink from a USB stick.

---

> **Windows SmartScreen note (both options):** SC-navLink is unsigned, so Windows can show a *"Windows protected your PC"* dialog when you start SC-navLink the first time. Click **More info**. Then click **Run anyway**. Or use the **Unblock** step above. Any Defender flag is a false positive for an unsigned app.
...
---
*SC-navLink is not affiliated with, endorsed by, or sponsored by Cloud Imperium Games or Roberts Space Industries. Star Citizen®, Roberts Space Industries® and Cloud Imperium® are registered trademarks of Cloud Imperium Rights LLC.*
